...
set.seed(12345)
outputPath="./Heatmap_lasso_lm_beta=1"
if(!dir.exists(outputPath)){
  dir.create(outputPath)
}

p=35  ###number of covarites
n=20000
true.mean=0
true.sd=1
beta=1
tau=0.8
par=c(0,rep(beta,5),rep(beta,7),rep(0,3),rep(beta,6),
      rep(0,4),rep(beta,5),rep(beta,3),rep(0,2)) 
...
for(i in 1:length(r_vec)){
  for(j in 6:6){
    temp=BLB_la(data,0.7,r_vec[i],tau,s=s_vec[j],family="gaussian")
